...
#define preParseTOKENTYPE  TokenPre 
typedef union {
  int yyinit;
...
#endif
#define preParseARG_SDECL  SodiumSession *session ;
#define preParseARG_PDECL , SodiumSession *session 
#define preParseARG_FETCH  SodiumSession *session  = yypParser->session 
...
#line 66 "lemon.pre.yy"
{
    ...
    if (isExists) {
        stopPreParserWithMessage(session, page, ETEXT(ERR_NAME_ALREADY_IN_USE), yymsp[-1].minor.yy13.tokenStr);
    } else {
        PreObjectVariable *obj = createPreObjectVariableVarchar(session, PRE_VARIABLE_TYPE_VARCHAR, yymsp[-1].minor.yy13.tokenStr, NULL);
        createPreObjectGeneric(session, page, yymsp[-1].minor.yy13.tokenStr, POST_OBJECT_GENERIC_TYPE_VARIABLE, obj);
        mkDebug(session, SESSION_LOG_DETAIL_HTSQL_VARIABLE, "\n", "Global variable created (char): '", yymsp[-1].minor.yy13.tokenStr, "'", NULL);
    }
...
#line 79 "lemon.pre.yy"
{
    ...
    PreObjectGeneric *isExists = findPreGenericObjectByName(session, page, yymsp[-1].minor.yy13.tokenStr);
    if (isExists) {
        stopPreParserWithMessage(session, page, "Değişken daha önce tanımlandı : ", yymsp[-1].minor.yy13.tokenStr);
    } else {
        PreObjectVariable *obj = createPreObjectVariableVarchar(session, PRE_VARIABLE_TYPE_BOOL, yymsp[-1].minor.yy13.tokenStr, NULL);
        createPreObjectGeneric(session, page, yymsp[-1].minor.yy13.tokenStr, POST_OBJECT_GENERIC_TYPE_VARIABLE, obj);
        mkDebug(session, SESSION_LOG_DETAIL_HTSQL_VARIABLE, "\n", "Global variable created (char): '", yymsp[-1].minor.yy13.tokenStr, "'", NULL);
    }
    preTokenDestructor(session, yymsp[-1].minor.yy13);
...
#line 92 "lemon.pre.yy"
{
    ...
    if (isExists) {
        ...
        PreObjectVariable *obj = createPreObjectVariableNumber(session, PRE_VARIABLE_TYPE_NUMBER, yymsp[-1].minor.yy13.tokenStr, 0);
        createPreObjectGeneric(session, page, yymsp[-1].minor.yy13.tokenStr, POST_OBJECT_GENERIC_TYPE_VARIABLE, obj);
        mkDebug(session, SESSION_LOG_DETAIL_HTSQL_VARIABLE, "\n", "Global variable created (int): '", yymsp[-1].minor.yy13.tokenStr, "'", NULL);
    }
    preTokenDestructor(session, yymsp[-1].minor.yy13);
}
...
#line 105 "lemon.pre.yy"
{
    ...
    if (isExists) {
        ...
        PreObjectVariable *obj = createPreObjectVariableDate(session, PRE_VARIABLE_TYPE_DATE, yymsp[-1].minor.yy13.tokenStr, NULL);
        createPreObjectGeneric(session, page, yymsp[-1].minor.yy13.tokenStr, POST_OBJECT_GENERIC_TYPE_VARIABLE, obj);
        mkDebug(session, SESSION_LOG_DETAIL_HTSQL_VARIABLE, "\n", "Global variable created (date): '", yymsp[-1].minor.yy13.tokenStr, "'", NULL);
    }
    preTokenDestructor(session, yymsp[-1].minor.yy13);
}
...
#line 118 "lemon.pre.yy"
{
	...
	if (isExists) {
		...
		PreObjectVariable *obj = createPreObjectVariableRedis(session, yymsp[-1].minor.yy13.tokenStr);
		createPreObjectGeneric(session, page, yymsp[-1].minor.yy13.tokenStr, POST_OBJECT_GENERIC_TYPE_VARIABLE, obj);
		mkDebug(session, SESSION_LOG_DETAIL_HTSQL_VARIABLE, "\n", "Global variable created (redis): '", yymsp[-1].minor.yy13.tokenStr, "'", NULL);
	}
	preTokenDestructor(session, yymsp[-1].minor.yy13);
}
...
#line 192 "lemon.pre.yy"
{
	...
	if (currentGenericObject) {
		char *bodyBegin = yymsp[-2].minor.yy0.tokenStr;
		char *bodyItself = yymsp[-1].minor.yy49;
		char *bodyEnd = yymsp[0].minor.yy0.tokenStr;
...
#line 210 "lemon.pre.yy"
{
	...
	if (currentGenericObject) {
		char *body = mkStrcat(session->heapHandle, __FILE__, __LINE__, yymsp[-1].minor.yy0.tokenStr, yymsp[0].minor.yy0.tokenStr, NULL);
		__setFunctionBody(session, page, currentGenericObject, body);
		mkFree(session->heapHandle, body);
	}
