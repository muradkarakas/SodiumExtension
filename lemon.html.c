...
#define htmlParseTOKENTYPE  TokenHtml 
typedef union {
  int yyinit;
...
#endif
#define htmlParseARG_SDECL  SodiumSession *session ;
#define htmlParseARG_PDECL , SodiumSession *session 
#define htmlParseARG_FETCH  SodiumSession *session  = yypParser->session 
...
#line 295 "lemon.html.yy"
{
    ...
    htmlTokenDestructor(session, yymsp[-3].minor.yy0);
    htmlTokenDestructor(session, yymsp[-2].minor.yy0);
    htmlTokenDestructor(session, yymsp[-1].minor.yy0);
	htmlTokenDestructor(session, yymsp[0].minor.yy0);
	if (name)
...
#line 2112 "lemon.html.c"
      ...
      case 28: /* tagdatablockpropertysourcename ::= SPACE PROP_BLOCK_DATA_SOURCE_NAME ASSIGMENT */ yytestcase(yyruleno==28);
...
#line 878 "lemon.html.yy"
{
    HTSQLPage *page = GetCurrentPage(session);
	//if (page->bInTableBody > 0) {
		char *str1 = getCreateNTString(session, yymsp[-2].minor.yy0);
		char *str2 = getCreateNTString(session, yymsp[-1].minor.yy0);
		char *str3 = getCreateNTString(session, yymsp[0].minor.yy0);
